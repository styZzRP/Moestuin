...
const Plus = mk("plus"), X = mk("x"), Trash2 = mk("trash"), Sprout = mk("sprout"),
  Scissors = mk("scissors"), Droplets = mk("droplets"), Apple = mk("apple"),
  NotebookPen = mk("notebook"), Pencil = mk("pencil"), Move = mk("move"),
  Grid3x3 = mk("grid"), ChevronLeft = mk("chevronLeft"), Eye = mk("eye"),
  Bug = mk("bug"), Camera = mk("camera"), Search = mk("search"), Archive = mk("archive");
...
function MoestuinApp() {
  ...
  const updateBed = (id, patch) =>
    persist({ ...state, beds: state.beds.map((b) => (b.id === id ? { ...b, ...patch } : b)) });

  const removeBed = (id) => {
    const bed = state.beds.find((b) => b.id === id);
    const toArchive = (state.crops[id] || []).map((c) => ({
      ...c, bedName: bed?.name || "Onbekende bak", removedAt: today(),
    }));
  ...
  const archiveCrop = (bedId, cropId) => {
    ...
    if (!crop) return;
    const archived = { ...crop, bedName: bed?.name || "Onbekende bak", removedAt: today() };
    persist({
      ...state,
  ...
  const deleteArchived = (cropId) =>
    persist({ ...state, archive: state.archive.filter((c) => c.id !== cropId) });

  if (!loaded) {
    return (
      <div style={S.loading}>
        <Sprout size={28} color="#6b8e3d" />
        <span style={{ marginLeft: 10 }}>Moestuin laden…</span>
      </div>
    );
  }
    ...
    <div style={S.root}>
      ...
      <header style={S.header} className="app-header">
        <div style={S.brand}>
          ...
          <div>
            <div style={S.brandName}>De Moestuin</div>
            <div style={S.brandSub}>{state.beds.length} bak{state.beds.length === 1 ? "" : "ken"}</div>
          </div>
        </div>
        <div style={S.headerActions} className="app-header-actions">
          <button
            style={{ ...S.btnGhost, ...(editLayout ? S.btnGhostActive : {}) }}
            onClick={() => setEditLayout((v) => !v)}
          >
            {editLayout ? <Grid3x3 size={16} /> : <Move size={16} />}
            <span className="btn-label">{editLayout ? "Klaar met indelen" : "Indeling wijzigen"}</span>
          </button>
          <button style={S.btnPrimary} onClick={addBed}>
            <Plus size={16} /> <span className="btn-label">Bak toevoegen</span>
          </button>
        </div>
      </header>

      {saveError && (
        <div style={S.saveWarn}>
          ⚠️ Opslaan lukt niet — de opslagruimte zit waarschijnlijk vol (vaak door veel foto's).
          Verwijder een paar foto's of gearchiveerde groenten om weer te kunnen opslaan.
        </div>
      )}

      <div style={S.searchWrap} className="app-search">
        <Search size={17} style={{ color: "#9a8c66", flexShrink: 0 }} />
        <input
          value={query}
        ...
        />
        {query && (
          <button style={S.iconBtn} onClick={() => setQuery("")} aria-label="Wissen"><X size={18} /></button>
        )}
      </div>
      ...
      <main style={S.main} className="app-main">
        {q ? (
          <SearchResults
            ...
            onOpenBed={(id) => { setQuery(""); setSelectedBed(id); }}
            onViewArchived={(crop) => setArchiveCropView(crop)}
          />
        ) : state.beds.length === 0 ? (
          <EmptyGarden onAdd={addBed} onLoadTemplate={() => persist(buildInitialGarden())} />
        ) : (
          <GardenCanvas
            beds={state.beds}
            cropsFor={cropsFor}
      ...
      {archiveCropView && (
        <ArchivedCropPanel
          item={archiveCropView}
          onClose={() => setArchiveCropView(null)}
          onDelete={() => { deleteArchived(archiveCropView.id); setArchiveCropView(null); }}
        />
      )}
      ...
      {bed && (
        <BedPanel
          bed={bed}
          crops={cropsFor(bed.id)}
          editLayout={editLayout}
          onClose={() => setSelectedBed(null)}
          onSetCrops={(c) => setCrops(bed.id, c)}
          onArchiveCrop={(cropId) => archiveCrop(bed.id, cropId)}
          onRenameBed={(name) => updateBed(bed.id, { name })}
          onRemoveBed={() => removeBed(bed.id)}
        />
      )}
    </div>
  );
...
function EmptyGarden({ onAdd, onLoadTemplate }) {
  return (
    <div style={S.empty}>
      <div style={S.emptyArt}>🌱</div>
      <h2 style={S.emptyTitle}>Je tuin is nog leeg</h2>
      <p style={S.emptyText}>
        Begin met de indeling van jouw tuin (bedden, kas en verhoogde bakken), of start met één lege
        bak. Je kunt alles daarna verslepen, vergroten en hernoemen.
      </p>
      <div style={{ display: "flex", gap: 10, justifyContent: "center", flexWrap: "wrap" }}>
        <button style={S.btnPrimary} onClick={onLoadTemplate}>
          <Grid3x3 size={16} /> Mijn tuinindeling laden
        </button>
        <button style={S.btnGhost} onClick={onAdd}>
          <Plus size={16} /> Lege bak aanmaken
        </button>
      </div>
    </div>
  );
...
function GardenCanvas({ beds, cropsFor, editLayout, onSelect, onUpdateBed }) {
  ...
  const onCanvasDown = (e) => {
    if (e.target.closest("[data-bed]")) return;
  ...
  const onTouchStart = (e) => {
    if (e.touches.length === 2) { setDrag(null); pinch.current = { d: dist(e.touches) }; }
  };
  const onTouchMove = (e) => {
  ...
  const onWheel = (e) => {
    if (!e.ctrlKey && !e.metaKey) return; // alleen met ctrl/cmd zoomen, anders normaal scrollen
    e.preventDefault();
    const rect = wrapRef.current.getBoundingClientRect();
  ...
  return (
    <div style={S.canvasOuter}>
      <div
        ref={wrapRef}
        data-canvas
        style={{ ...S.viewport, cursor: drag?.mode === "pan" ? "grabbing" : "default" }}
        ...
        onWheel={onWheel}
      >
        <div style={{ ...S.world, width: WORLD_W, height: WORLD_H,
          ...
          {beds.map((bed) => {
            ...
            return (
              <div
                ...
                onClick={() => !editLayout && !drag && onSelect(bed.id)}
                className="bed-tile"
                style={{
                  ...
                  boxShadow: drag?.id === bed.id ? "0 14px 32px rgba(40,30,15,.35)" : S.bed.boxShadow,
                }}
              >
                <div style={S.soilTexture} />
                <div style={S.bedName}>{bed.name}</div>
                <div style={S.bedCrops}>
                  {crops.length === 0 ? (
                    <span style={S.bedEmpty}>leeg</span>
                  ) : (
                    crops.slice(0, 4).map((c) => <span key={c.id} style={S.cropChip}>{c.name}</span>)
                  )}
                  {crops.length > 4 && <span style={S.cropChip}>+{crops.length - 4}</span>}
                </div>
                {editLayout && <div style={S.moveBadge}><Move size={13} /></div>}
                {editLayout && (
                  <div style={S.resizeHandle} onPointerDown={(e) => onResizeDown(e, bed)}
                    onClick={(e) => e.stopPropagation()} aria-label="Formaat aanpassen" />
                )}
              </div>
            );
          })}
      ...
      <div style={S.zoomBar}>
        <button style={S.zoomBtn} onClick={() => zoomBy(1.2)} aria-label="Inzoomen">+</button>
        <button style={S.zoomBtn} onClick={() => zoomBy(0.83)} aria-label="Uitzoomen">−</button>
        <button style={S.zoomReset} onClick={() => {
          const el = wrapRef.current; const w = el.clientWidth, h = el.clientHeight;
          const scale = Math.min(w / WORLD_W, h / WORLD_H, 1);
          setView({ scale, tx: (w - WORLD_W * scale) / 2, ty: 16 });
        }} aria-label="Passend">⤢</button>
      </div>
...
function BedPanel({ bed, crops, editLayout, onClose, onSetCrops, onArchiveCrop, onRenameBed, onRemoveBed }) {
  ...
  const updateCrop = (id, patch) =>
    onSetCrops(crops.map((c) => (c.id === id ? { ...c, ...patch } : c)));
  const removeCrop = (id) => {
    onArchiveCrop(id);
    if (openCrop === id) setOpenCrop(null);
  };

  return (
    <>
      <div style={S.scrim} onClick={onClose} />
      <aside style={S.panel} className="panel-in">
        <div style={S.panelHead}>
          <button style={S.iconBtn} onClick={onClose} aria-label="Sluiten"><ChevronLeft size={20} /></button>
          {editName ? (
            <input
              autoFocus
              value={nameDraft}
              onChange={(e) => setNameDraft(e.target.value)}
              onBlur={() => { onRenameBed(nameDraft.trim() || bed.name); setEditName(false); }}
              onKeyDown={(e) => e.key === "Enter" && e.currentTarget.blur()}
              style={S.nameInput}
            />
          ) : (
            <button style={S.panelTitle} onClick={() => setEditName(true)}>
              {bed.name} <Pencil size={14} style={{ opacity: 0.5 }} />
            </button>
          )}
        ...
        <div style={S.panelBody}>
          <div style={S.addRow}>
            <input
              ...
              placeholder="Groente toevoegen, bijv. Tomaat"
              style={S.input}
            />
            <button style={S.btnPrimary} onClick={addCrop}><Plus size={16} /></button>
          </div>

          {crops.length === 0 ? (
            <p style={S.panelEmpty}>Nog geen groenten in deze bak. Voeg er hierboven een toe.</p>
          ) : (
            crops.map((crop) => (
              <CropCard
                key={crop.id}
                crop={crop}
                open={openCrop === crop.id}
                onToggle={() => setOpenCrop(openCrop === crop.id ? null : crop.id)}
                onUpdate={(patch) => updateCrop(crop.id, patch)}
                onRemove={() => removeCrop(crop.id)}
              />
            ))
          )}
          ...
          <div style={S.dangerZone}>
            {confirmDelBed ? (
              <div style={S.confirmRow}>
                <span style={S.confirmText}>"{bed.name}" verwijderen? Groenten gaan naar het archief.</span>
                <div style={S.confirmBtns}>
                  <button style={S.btnGhostSmall} onClick={() => setConfirmDelBed(false)}>Annuleren</button>
                  <button style={{ ...S.btnConfirm, background: "#a13d2d" }} onClick={() => { setConfirmDelBed(false); onRemoveBed(); }}>
                    <Trash2 size={13} /> Verwijderen
                  </button>
                </div>
              </div>
            ) : (
              <button style={S.btnDanger} onClick={() => setConfirmDelBed(true)}>
                <Trash2 size={15} /> Deze bak verwijderen
              </button>
            )}
          </div>
        </div>
...
function CropCard({ crop, open, onToggle, onUpdate, onRemove }) {
  ...
  const addLog = () => {
    ...
    setLogNote(""); setLogPhotos([]); setDiseaseOther("");
  };
  ...
  return (
    <div style={S.cropCard}>
      <button style={S.cropHead} onClick={onToggle}>
        <div style={S.cropHeadLeft}>
          <Sprout size={17} color="#6b8e3d" />
          <span style={S.cropName}>{crop.name}</span>
        </div>
        <div style={S.cropMeta}>
          {LOG_TYPES.map(({ key, icon: Icon, color }) => {
            const d = lastByType(key);
            return d ? (
              <span key={key} style={{ ...S.miniStat, color }}>
                <Icon size={12} /> {fmtShort(d)}
              </span>
            ) : null;
          })}
        </div>
      ...
      {open && (
        <div style={S.cropDetail}>
          <div style={S.sectionLabel}>Logboek</div>
          {crop.logs.length === 0 && <p style={S.tinyEmpty}>Nog geen gebeurtenissen.</p>}
            ...
            {crop.logs.map((l) => {
              ...
              return (
                <div key={l.id} style={S.logRow}>
                  <span style={{ ...S.logDot, background: t.color }}><Icon size={12} color="#fff" /></span>
                  <div style={{ flex: 1 }}>
                    <div style={S.logTop}>
                      <strong style={{ color: t.color }}>
                        {t.label}{l.type === "ziekte" && l.disease ? `: ${l.disease}` : ""}
                      </strong>
                      <span style={S.logDate}>{fmtLong(l.date)}</span>
                    </div>
                    {l.note && <div style={S.logNote}>{l.note}</div>}
                    {l.photos && l.photos.length > 0 && (
                      <div style={S.photoStrip}>
                        {l.photos.map((src, i) => (
                          <img key={i} src={src} alt="" style={S.photoThumb} onClick={() => setViewPhoto(src)} />
                        ))}
                      </div>
                    )}
                  </div>
                  <button style={S.logDel} onClick={() => removeLog(l.id)} aria-label="Verwijderen"><X size={14} /></button>
                </div>
              );
            })}
          </div>
          ...
          <div style={S.logForm}>
            <div style={S.typeRow}>
              {LOG_TYPES.map(({ key, label, icon: Icon, color }) => (
                <button
                  key={key}
            ...
            {logType === "ziekte" && (
              <div style={S.diseaseRow}>
                <select value={disease} onChange={(e) => setDisease(e.target.value)} style={S.select}>
                ...
                {disease === "Anders…" && (
                  <input
                    ...
                    placeholder="Welke ziekte of plaag?"
                    style={S.input}
                  />
                )}
              </div>
            )}
            <div style={S.logFormBottom}>
              <input type="date" value={logDate} onChange={(e) => setLogDate(e.target.value)} style={S.dateInput} />
              <button style={S.btnPrimary} onClick={addLog}><Plus size={16} /> Toevoegen</button>
            </div>
            <textarea
              value={logNote}
              onChange={(e) => setLogNote(e.target.value)}
              onKeyDown={(e) => (e.key === "Enter" && (e.metaKey || e.ctrlKey)) && addLog()}
              placeholder="Notitie of bevinding…"
              style={S.logTextarea}
              rows={3}
            />
            {logPhotos.length > 0 && (
              <div style={S.photoStrip}>
                {logPhotos.map((src, i) => (
                  <div key={i} style={S.previewWrap}>
                    ...
                    <button style={S.photoRemove} onClick={() => setLogPhotos((p) => p.filter((_, j) => j !== i))}>
                      <X size={11} />
                    </button>
                  </div>
                ))}
            ...
            <label style={S.photoBtn}>
              <Camera size={15} /> {busy ? "Bezig…" : "Foto toevoegen"}
              <input type="file" accept="image/*" multiple onChange={onPickPhotos} style={{ display: "none" }} disabled={busy} />
            </label>
          </div>

          {confirmRemove ? (
            <div style={S.confirmRow}>
              <span style={S.confirmText}>Uit bak halen? Logboek blijft bewaard in archief.</span>
              <div style={S.confirmBtns}>
                <button style={S.btnGhostSmall} onClick={() => setConfirmRemove(false)}>Annuleren</button>
                <button style={S.btnConfirm} onClick={() => { setConfirmRemove(false); onRemove(); }}>
                  <Archive size={13} /> Uit bak halen
                </button>
              </div>
            </div>
          ) : (
            <button style={S.btnDangerSmall} onClick={() => setConfirmRemove(true)}>
              <Archive size={13} /> Uit bak halen (logboek bewaren)
            </button>
          )}

          {viewPhoto && (
            <div style={S.lightbox} onClick={() => setViewPhoto(null)}>
              <img src={viewPhoto} alt="" style={S.lightboxImg} />
            </div>
          )}
        </div>
      )}
...
function SearchResults({ current, archived, onOpenBed, onViewArchived }) {
  ...
  if (total === 0) {
    return (
      <div style={S.searchEmpty}>
        <Search size={32} color="#bcae88" />
        <p style={{ margin: "12px 0 0", color: "#8a7d5c" }}>
          Geen groente gevonden — niet in een bak en niet in het archief.
        </p>
      </div>
    );
  }
  return (
    <div style={S.results}>
      {current.length > 0 && (
        <>
          <div style={S.resultsHead}><Sprout size={15} color="#6b8e3d" /> Nu in de tuin</div>
          {current.map(({ crop, bedId, bedName }) => (
            <button key={crop.id} style={S.resultCard} onClick={() => onOpenBed(bedId)}>
              <div style={S.resultMain}>
                <span style={S.resultName}>{crop.name}</span>
                <span style={S.resultBed}>{bedName}</span>
              </div>
              <span style={S.resultMeta}>{crop.logs.length} logboek-item{crop.logs.length === 1 ? "" : "s"}</span>
            </button>
          ))}
        </>
      )}
      {archived.length > 0 && (
        <>
          <div style={{ ...S.resultsHead, marginTop: current.length ? 22 : 0 }}>
            <Archive size={15} color="#9a7b3d" /> Eerder geteeld (archief)
          </div>
          {archived.map(({ crop, bedName, removedAt }) => (
            <button key={crop.id} style={{ ...S.resultCard, ...S.resultCardArchived }} onClick={() => onViewArchived(crop)}>
              <div style={S.resultMain}>
                <span style={S.resultName}>{crop.name}</span>
                <span style={S.resultBed}>was in {bedName}</span>
              </div>
              <span style={S.resultMeta}>
                {crop.logs.length} item{crop.logs.length === 1 ? "" : "s"} · gestopt {fmtShort(removedAt)}
              </span>
            </button>
          ))}
        </>
      ...
      <aside style={S.panel} className="panel-in">
        <div style={S.panelHead}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={S.archivedSub}>
              <Archive size={12} /> was in {item.bedName} · gestopt {fmtLong(item.removedAt)}
            </div>
          </div>
          <button style={S.iconBtn} onClick={onClose} aria-label="Sluiten"><X size={20} /></button>
        </div>
        <div style={S.panelBody}>
          <div style={S.archivedBanner}>
            Deze groente staat niet meer in een bak. Het logboek is bewaard en alleen-lezen.
          </div>
          <div style={S.sectionLabel}>Logboek</div>
          <LogTimeline logs={item.logs} onPhoto={setViewPhoto} />
          <div style={S.dangerZone}>
            {confirmDel ? (
              <div style={S.confirmRow}>
                <span style={S.confirmText}>Logboek van "{item.name}" definitief verwijderen? Dit kan niet ongedaan worden.</span>
                <div style={S.confirmBtns}>
                  <button style={S.btnGhostSmall} onClick={() => setConfirmDel(false)}>Annuleren</button>
                  <button style={{ ...S.btnConfirm, background: "#a13d2d" }} onClick={onDelete}>
                    <Trash2 size={13} /> Definitief verwijderen
                  </button>
                </div>
              </div>
            ) : (
              <button style={S.btnDanger} onClick={() => setConfirmDel(true)}>
                <Trash2 size={15} /> Definitief verwijderen uit archief
              </button>
            )}
          </div>
        </div>
...
function LogTimeline({ logs, onPhoto }) {
  if (!logs || logs.length === 0) return <p style={S.tinyEmpty}>Geen logboek-items.</p>;
  return (
    <div style={S.timeline}>
      {logs.map((l) => {
          ...
          <div key={l.id} style={S.logRow}>
            ...
            <div style={{ flex: 1 }}>
              ...
              {l.photos && l.photos.length > 0 && (
                <div style={S.photoStrip}>
                  {l.photos.map((src, i) => (
                    <img key={i} src={src} alt="" style={S.photoThumb} onClick={() => onPhoto(src)} />
                  ))}
                </div>
              )}
            </div>
...
const S = {
  ...
  brandSub: { fontSize: 12.5, color: "#8a7d5c", marginTop: 3 },
